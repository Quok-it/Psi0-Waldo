...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 384) -> None:
        ...
        default_dir = os.path.join(parent_folder_path, 'models', 'bak', 'dino-siglip')
        dino_siglip_dir = os.environ.get("DINO_SIGLIP_DIR", default_dir)
        ...
        if not os.path.exists(dino_model_path) or not os.path.exists(siglip_model_path):
            raise FileNotFoundError(
                "Missing DINO/SigLIP weights. Expected:\n"
                f"  DINO: {dino_model_path}\n"
                f"  SigLIP: {siglip_model_path}\n"
                "Set DINO_SIGLIP_DIR to the folder containing the two model subfolders."
            )
