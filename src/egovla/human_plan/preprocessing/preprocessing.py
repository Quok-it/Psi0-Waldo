...
def preprocess_vla(
  ...
  if raw_action_label:
    ...
    label_dim = traj_action_output_dim
    if label_dim is None or label_dim <= 0:
      label_dim = raw_label.shape[-1]
    raw_label = raw_label.reshape(-1, label_dim)
    label_mask = label_mask.reshape(-1, label_dim)
    label_mask_for_tokenizer = label_mask[:, 0]
    # Ensure numpy mask for action_tokenizer
